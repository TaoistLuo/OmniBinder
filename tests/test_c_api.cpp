...
static const uint32_t HASH_ECHO = 0xCAFE0001u;
static const uint32_t HASH_GET_NAME = 0xCAFE0002u;
static const uint32_t HASH_ECHO_BYTES = 0xCAFE0003u;
...
protected:
    ...
    static void SetUpTestSuite() {
        ...
        omni_service_add_method_ex(server_ctx_->service, METHOD_ECHO, "Echo", "std::string", "std::string", HASH_ECHO);
        omni_service_add_method_ex(server_ctx_->service, METHOD_GET_NAME, "GetName", "", "std::string", HASH_GET_NAME);
        omni_service_add_method_ex(server_ctx_->service, METHOD_ECHO_BYTES, "EchoBytes", "std::vector<uint8_t>", "std::vector<uint8_t>", HASH_ECHO_BYTES);
...
TEST_F(CApiTest, RegisterInvokeAndStats) {
    ...
    ASSERT_EQ(omni_runtime_invoke(runtime, "CService", IFACE_ID, METHOD_ECHO, HASH_ECHO, req, resp, 3000), 0);
...
TEST_F(CApiTest, StringAndBytesMethods) {
    ...
    ASSERT_EQ(omni_runtime_invoke(runtime, "CService", IFACE_ID, METHOD_GET_NAME, HASH_GET_NAME, req, resp, 3000), 0);
    uint32_t name_len = 0;
    ...
    ASSERT_EQ(omni_runtime_invoke(runtime, "CService", IFACE_ID, METHOD_ECHO_BYTES, HASH_ECHO_BYTES, req, resp, 3000), 0);
    uint32_t out_len = 0;
