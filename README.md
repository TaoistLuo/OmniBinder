...
> **注意**：双阶段脚本要求调用时通过 `CC/CXX` 指定目标编译器；脚本会在 host stage 主动清除这些变量，再在 cross stage 使用它们。
>
...
完整、可直接生成的定义位于 [`examples/sensor_service.bidl`](examples/sensor_service.bidl)，其导入的公共类型位于
[`examples/common_types.bidl`](examples/common_types.bidl)。下面的命令从仓库根目录运行；相比复制易漂移的片段，
请直接修改并编译这两个真实示例文件。
...
> **注意**：可用测试由平台和构建配置决定；Windows 构建会跳过依赖 POSIX/Linux 行为的部分测试。请以 `ctest -N` 的当前输出为准。
