...
测试集合由平台和构建配置决定。Windows 会跳过 `test_diagnostics_watch`、`test_idl_mismatch` 和依赖 g++ 生成代码流水线的 `test_generated_runtime_integration`；Python、`omni-idlc` target 等条件也会影响注册项。请以当前构建的 `ctest --test-dir build -N` 为准。
...
测试数量随平台和配置变化，不应写死。使用以下命令查看当前构建实际注册的测试：
...
- `test_error_logging`
- `test_longrun_tools`
- `test_owner_thread_executor`
- `test_omni_idlc_invalid_language`
- `test_omni_cli_info_formatter`
- `test_stats`
- `test_thread_safety`
- `test_sm_reconnect`
...
- RPC Add 延迟
- topic pub/sub 延迟
- CTest 通过显式 `-o` 参数更新仓库中的 `docs/performance-report.md`
...
通过 CTest 或以上带 `-o` 的手工命令成功执行后，会更新：
...
while true; do
  ...
  ./target/test/test_soak_runner 5 || break
  ./target/test/test_transient_disconnect_recovery 5 || break
  ctest --output-on-failure -R "test_c_api|test_omni_cli_type_codec|test_runtime_stats|test_error_logging" || break
...
- 如需持续更新仓库性能报告，另行执行 `./target/test/test_performance -o ../docs/performance-report.md`；`test_soak_runner` 的内部调用不承诺写入该路径
- 以下关键词是否持续增长：
  - `sm_connection_lost`
  - `sm_reconnect_begin`
  - `sm_reconnect_success`
